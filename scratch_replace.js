...
const enPath = 'c:\\Users\\Timoz\\WebProjects\\MyWebsite\\en\\index.html';
const ptPath = 'c:\\Users\\Timoz\\WebProjects\\MyWebsite\\pt\\index.html';
...
// Meta tags
html = html.replace(/<title>.*?<\/title>/g, '<title>ReCode — Desenvolvedor Mobile Independente</title>');
html = html.replace(/<meta name="description" content=".*?" \/>/g, '<meta name="description" content="ReCode — desenvolvedor independente de aplicativos móveis. Especializado em vibecoding: criação de aplicativos de alta qualidade para App Store e Google Play." />');
html = html.replace(/<meta name="keywords" content=".*?" \/>/g, '<meta name="keywords" content="ReCode, vibecoding, aplicativos móveis, KMP, Kotlin Multiplatform, Compose Multiplatform, MindHug" />');

html = html.replace(/<meta property="og:title" content=".*?" \/>/g, '<meta property="og:title" content="ReCode — Desenvolvedor Mobile Independente" />');
html = html.replace(/<meta property="og:description" content=".*?" \/>/g, '<meta property="og:description" content="Vibecoding: criação e publicação de aplicativos de alta qualidade nas maiores lojas do mundo." />');
html = html.replace(/<meta property="og:locale" content=".*?" \/>/g, '<meta property="og:locale" content="pt_BR" />');
...
fs.writeFileSync(ptPath, html, 'utf8');
console.log("Done");
